...
2026-08-21. Verification-only Stage 3 execution is authorized. It uses equal
command re-execution, order-significant result comparison and public-API probes
without adding WAL/replay infrastructure, state exposure or production test
hooks. OrderBook is an external frozen dependency and no OrderBook file or API
may change.
